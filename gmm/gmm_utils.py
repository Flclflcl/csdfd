...
def gmr_lyapunov(x, obs, Priors, Mu, P):
    ...
    for k in range(L + 1):
        ...
        if k == 0:
            ...
            V = Priors[k] * V_k
            Vx = Priors[k] * ((P_cur+P_cur.T).dot(x))   # 一阶导
            # V = V_k
            # Vx = (P_cur+P_cur.T).dot(x)    # 一阶导
        else:
            x_tmp = np.add(x, 3.5 *obs)
            # if obs.any() > 1e-6:
                # print('collision', obs[obs.any() > 1e-6])
            # x_tmp = x - np.tile(Mu[:, k], [nbData, 1]).T
            x_ori = x
            V_k = np.sum(P_cur.dot(x_ori)*x_tmp, axis=0)
            V_k[V_k < 0.] = 0.
            Priors_reshaped = np.reshape(Priors[k], [1])
            V += Priors_reshaped.dot(np.expand_dims(V_k ** 2, axis=0))
            temp = (1 * Priors_reshaped).dot(np.expand_dims(V_k, axis=0))

            # V += V_k ** 2
            # temp = V_k
            Vx = Vx + np.tile(temp, [d, 1]) * (P_cur.dot(x_ori) + P_cur.T.dot(x_tmp))
